...
props : 
    ...
    - name: icon
      type: Boolean
      values: true | false
      description: Determine if the component contains only one icon, by adding this props the component has an equal width and height.
      default: "false"
    ...
    - name: animationType
      type: Boolean
      values: true | false
      description: Change the type of animation, this props only works if you have the slot animate.
      default: "false"
...
You can change the full style towards *flat* of a button with the `flat` props that its value is a `boolean`
...
You can change the full style towards *border* of a button with the `border` props that its value is a `boolean` so just adding it will change the styles
...
You can change the full style towards *Gradient* of a button with the `gradient` props that its value is a `boolean`

:::tip Gradient Auto Color
  Gradient colors are automatically generated by the `color` props which in this example is the`primary` color
:::
...
Change the style of the button to a relief aspect with the `relief` props, the props is a `boolean`
...
Change the style of the button with the `transparent` props, the props is a `boolean`
...
Change the style of the button with the `shadow` props, the props is a `boolean`
...
Change the color of the Button component with the `color` props and the color value for example (**#fff**, **rgba (100,10,5)** or the **main colors of vuesax**), this will only affect the component and its value can be dynamic
...
If you need an icon-only button use the `icon` props, the props is a `boolean`

This props makes the button have a specific size making it the same width and height, do not use when it is icon and text
...
You can completely round the corners with the `circle` props, the props is a `boolean`
...
You can make all the corners completely straight with the `Square` props, the props is a `boolean`
...
Change the size of the entire button including padding, font-size and border with the `size` props
...
Now you can add a loading status with the `loading` props, the props is a `boolean`
...
Now you can add a status of sending or uploading data to the server with the `upload` props, the props is a `boolean`
...
You can make the button have the full width allowed with the `block` props, the props is a `boolean`
...
You can also change the type of animation with the `animation-type` props and the allowed values ​​are (`vertical`, `scale`,`rotate`)
...
You can make a float style button easily with the `Floating` props, the props is a `boolean`
